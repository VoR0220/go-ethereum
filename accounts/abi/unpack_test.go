...
func TestSimpleMethodUnpack(t *testing.T) {
	for i, test := range []struct {
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal bool in to int8",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal bool in to int16",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal bool in to int32",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal bool in to int64",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal bool in to uint8",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal bool in to uint16",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal bool in to uint32",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal bool in to uint64",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"*big.Int",
			"abi: cannot unmarshal bool in to *big.Int",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"string",
			"abi: cannot unmarshal bool in to string",
		},
		{
			`[ { "type": "bool" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"address",
			"abi: cannot unmarshal bool in to common.Address",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal uint8 in to int8",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal uint8 in to int16",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal uint8 in to int32",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal uint8 in to int64",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal uint8 in to uint16",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal uint8 in to uint32",
		},
		{
			`[ { "type": "uint8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal uint8 in to uint64",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal uint16 in to int8",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal uint16 in to int16",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal uint16 in to int32",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal uint16 in to int64",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal uint16 in to uint8",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal uint16 in to uint32",
		},
		{
			`[ { "type": "uint16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal uint16 in to uint64",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal uint32 in to int8",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal uint32 in to int16",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal uint32 in to int32",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal uint32 in to int64",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal uint32 in to uint8",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"uint64",
			"abi: cannot unmarshal uint32 in to uint64",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal uint64 in to int8",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal uint64 in to int16",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal uint64 in to int32",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal uint64 in to int64",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal uint64 in to uint8",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal uint64 in to uint16",
		},
		{
			`[ { "type": "uint64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal uint64 in to uint32",
		},
		{
			`[ { "type": "uint32" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal *big.Int in to int8",
		},
		{
			`[ { "type": "uint256" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int32",
			"abi: cannot unmarshal *big.Int in to int32",
		},
		{
			`[ { "type": "uint256" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal *big.Int in to int64",
		},
		{
			`[ { "type": "uint256" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal *big.Int in to uint8",
		},
		{
			`[ { "type": "uint256" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"uint64",
			"abi: cannot unmarshal *big.Int in to uint64",
		},
		{
			`[ { "type": "int256" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"uint8",
			"abi: cannot unmarshal int8 in to uint8",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal int8 in to int16",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal int8 in to int32",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal int8 in to int64",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal int8 in to uint16",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal int8 in to uint32",
		},
		{
			`[ { "type": "int8" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal int8 in to uint64",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal int16 in to int8",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal int16 in to uint16",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal int16 in to int32",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal int16 in to int64",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal int16 in to uint8",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal int16 in to uint32",
		},
		{
			`[ { "type": "int16" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal int16 in to uint64",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal int32 in to int8",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"uint32",
			"abi: cannot unmarshal int32 in to uint32",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int64",
			"abi: cannot unmarshal int32 in to int64",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal int32 in to uint8",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal int32 in to uint16",
		},
		{
			`[ { "type": "int32" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal int32 in to uint64",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
		...
		},
		{
			...
			nil,
			"int8",
			"abi: cannot unmarshal int64 in to int8",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int16",
			"abi: cannot unmarshal int64 in to int16",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"int32",
			"abi: cannot unmarshal int64 in to int32",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint64",
			"abi: cannot unmarshal int64 in to uint64",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint8",
			"abi: cannot unmarshal int64 in to uint8",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint16",
			"abi: cannot unmarshal int64 in to uint16",
		},
		{
			`[ { "type": "int64" } ]`,
			pad([]byte{1}, 32, true),
			nil,
			"uint32",
			"abi: cannot unmarshal int64 in to uint32",
		},
		{
			`[ { "type": "address" } ]`,
